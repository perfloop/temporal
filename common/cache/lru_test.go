...
func TestCache_PinnedEntryCountTracksPositiveReferences(t *testing.T) {
	...
	err := quick.Check(func(randomOperations []uint8) bool {
		...
		operations := append([]uint8{
			...
			4,
			17, // Release key 1, then insert key 5 and evict key 1.
			5,
			10, 18, 18, // Re-pin then fully release key 2.
			10, 98, // Re-pin key 2, update its size, and release it.
			27, // Delete the still-pinned key 3.
		}, randomOperations...)
