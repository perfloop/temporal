...
cleanup() {
	"$postgres_bin/pg_ctl" -D "$data_dir" -m immediate stop >>"$log_file" 2>&1 || true
	rm -rf "$work_dir"
}
trap cleanup EXIT

"$postgres_bin/initdb" -D "$data_dir" --no-locale --encoding=UTF8 --auth=trust --username=temporal >"$log_file" 2>&1
"$postgres_bin/pg_ctl" -D "$data_dir" -l "$log_file" -o "-h 127.0.0.1 -p $port -F" -w start >>"$log_file" 2>&1
