...
# PostgreSQL imposes a 107-byte UNIX-socket pathname limit. The socket is
# control scratch, while the data directory remains worktree-local.
socket_dir=$(mktemp -d "${TMPDIR:-/tmp}/temporal-eager-start-pg.XXXXXX")
port=$(python3 - <<'PY'
import socket
...
cleanup() {
	...
	rm -rf "$socket_dir" "$work_dir"
	return "$status"
}
trap cleanup EXIT

"$postgres_bin/initdb" -D "$data_dir" --no-locale --encoding=UTF8 --auth=trust --username=temporal >"$log_file" 2>&1
"$postgres_bin/pg_ctl" -D "$data_dir" -l "$log_file" -o "-h 127.0.0.1 -k $socket_dir -p $port -F" -w start >>"$log_file" 2>&1
...
# The request path is serial. A single Go scheduler worker avoids scheduler
# contention from obscuring the synchronous PostgreSQL read being measured.
export GOMAXPROCS=1
...
if [[ -n ${PERFLOOP_BENCH_BIN:-} ]]; then
	"$PERFLOOP_BENCH_BIN" -test.bench='^BenchmarkEagerStartConflict$' -test.run='^$' -test.benchtime=20s -test.benchmem
else
	go test -bench='^BenchmarkEagerStartConflict$' -run='^$' -count=1 -benchtime=20s -benchmem ./service/history
fi
