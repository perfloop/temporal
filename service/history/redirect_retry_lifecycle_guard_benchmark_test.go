...
const (
	redirectLifecycleGuardInitialBuildID = "dispatch-build-before-redirect"
	redirectLifecycleGuardTargetBuildID  = "dispatch-build-after-redirect"
)

type redirectRetryLifecycleGuardBenchmarkFixture struct {
	b *testing.B
	...
	matchingClient   *redirectRetryLifecycleGuardMatchingClient
...
type redirectRetryLifecycleGuardWork struct {
	transferTasks []*tasks.ActivityTask
	timerTasks    []*tasks.ActivityRetryTimerTask
}

// redirectRetryLifecycleGuardMatchingClient makes the benchmark consume every
...
type redirectRetryLifecycleGuardMatchingClient struct {
	matchingservice.MatchingServiceClient

	calls int
}

func (c *redirectRetryLifecycleGuardMatchingClient) AddActivityTask(
	_ context.Context,
	request *matchingservice.AddActivityTaskRequest,
	_ ...grpc.CallOption,
) (*matchingservice.AddActivityTaskResponse, error) {
	if request.GetVersionDirective().GetAssignedBuildId() != redirectLifecycleGuardTargetBuildID {
		return nil, fmt.Errorf("matching directive = %q, want %q", request.GetVersionDirective().GetAssignedBuildId(), redirectLifecycleGuardTargetBuildID)
	}
	c.calls++
	return redirectRetryLifecycleGuardAddActivityTaskResponse, nil
}

var redirectRetryLifecycleGuardAddActivityTaskResponse = &matchingservice.AddActivityTaskResponse{}

func newRedirectRetryLifecycleGuardBenchmarkFixture(b *testing.B) *redirectRetryLifecycleGuardBenchmarkFixture {
	b.Helper()

	fixture := &redirectRetryLifecycleGuardBenchmarkFixture{
		b:           b,
	...
	fixture.matchingClient = &redirectRetryLifecycleGuardMatchingClient{}
...
type redirectRetryLifecycleGuardSetup struct {
...
func (f *redirectRetryLifecycleGuardBenchmarkFixture) prepareRetryLifecycle(width int) redirectRetryLifecycleGuardSetup {
	f.b.Helper()
	...
	mutableState.GetExecutionInfo().AssignedBuildId = redirectLifecycleGuardInitialBuildID
	...
	for i := 0; i < width; i++ {
		...
		retry.BuildIdInfo = &persistencespb.ActivityInfo_UseWorkflowBuildIdInfo_{
			UseWorkflowBuildIdInfo: &persistencespb.ActivityInfo_UseWorkflowBuildIdInfo{
				LastUsedBuildId: redirectLifecycleGuardInitialBuildID,
			},
		}
	...
	return redirectRetryLifecycleGuardSetup{
...
func (f *redirectRetryLifecycleGuardBenchmarkFixture) redirectAndPrepareDispatch(setup redirectRetryLifecycleGuardSetup) redirectRetryLifecycleGuardWork {
	if err := setup.mutableState.ApplyBuildIdRedirect(setup.triggerEventID, redirectLifecycleGuardTargetBuildID, 1); err != nil {
		f.b.Fatal(err)
	}
	...
	case len(transferTasks) == setup.width:
		return redirectRetryLifecycleGuardWork{transferTasks: transferTasks}
	case len(transferTasks) == 0 && len(setup.retryTimers) == setup.width:
		return redirectRetryLifecycleGuardWork{timerTasks: setup.retryTimers}
	default:
		...
		return redirectRetryLifecycleGuardWork{}
	}
}

func (f *redirectRetryLifecycleGuardBenchmarkFixture) dispatch(work redirectRetryLifecycleGuardWork) error {
...
func (f *redirectRetryLifecycleGuardBenchmarkFixture) clearMutableState(workflowKey definition.WorkflowKey) {
...
func (f *redirectRetryLifecycleGuardBenchmarkFixture) mustGenerateTaskID() int64 {
...
// BenchmarkRedirectedFutureRetryLifecycleGuard measures the full redirect-to-Matching
...
func BenchmarkRedirectedFutureRetryLifecycleGuard(b *testing.B) {
	for _, width := range []int{1, 10, 100} {
		b.Run(fmt.Sprintf("%d", width), func(b *testing.B) {
			fixture := newRedirectRetryLifecycleGuardBenchmarkFixture(b)
			b.ReportAllocs()

			var (
				err      error
				lastWork redirectRetryLifecycleGuardWork
			)
			for b.Loop() {
				b.StopTimer()
