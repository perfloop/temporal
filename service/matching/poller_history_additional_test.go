...
func TestPollerHistoryAdditional(t *testing.T) {
	...
	t.Run("pollMetadata HasPollerAfter true under repeated identical updates", func(t *testing.T) {
		t.Parallel()
		history := newPollerHistory(5 * time.Minute)

		identity := pollerIdentity("worker-sticky")
		metadata := &pollMetadata{}
		history.updatePollerInfo(identity, metadata)

		// Sleep for 15ms to simulate time increment
		time.Sleep(15 * time.Millisecond) //nolint:forbidigo

		// Update again
		history.updatePollerInfo(identity, metadata)

		// Check if getPollerInfo with accessTime of 10ms ago returns the poller
